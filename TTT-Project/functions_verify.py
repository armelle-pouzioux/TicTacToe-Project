...
def verify_win(player):
    ...
    if all(players[player] in board[coordinate] for coordinate in (0,3,6))\
     or all(players[player] in board[coordinate] for coordinate in (1,4,7))\
     or all(players[player] in board[coordinate] for coordinate in (2,5,8)):
        return True
    
    # rows combinations
    elif all(players[player] in board[coordinate] for coordinate in (0,1,2))\
     or all(players[player] in board[coordinate] for coordinate in (3,4,5))\
     or all(players[player] in board[coordinate] for coordinate in (6,7,8)):
        return True
